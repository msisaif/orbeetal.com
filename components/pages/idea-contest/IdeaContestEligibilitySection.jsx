...
export function IdeaContestEligibilitySection() {
  ...
  const cardMotion = useSafeMotion({ initial: { opacity: 0, y: 20 }, whileInView: { opacity: 1, y: 0 }, viewport: { once: true }, transition: { duration: 0.7, delay: 0.2 } });

  return (
    <section className="py-24 border-b border-white/5">
      <div className="container mx-auto px-6 md:px-12 text-center">
        <m.p {...taglineMotion} className="text-primary text-sm font-medium uppercase tracking-widest mb-4">
          Eligibility
        </m.p>
        <m.h2 {...titleMotion} className="text-4xl md:text-5xl font-display font-bold mb-12">
          Who Can <span className="text-gradient">Participate?</span>
        </m.h2>
        <m.div {...cardMotion} className="glass-panel border border-primary/20 rounded-2xl p-12 max-w-xl mx-auto">
