...
describe("data schemas", () => {
  ...
  it("validates portfolio project", () => {
    const result = portfolioProjectSchema.safeParse({
      ...
      preview: "/portfolio/test.webp",
    });
    expect(result.success).toBe(true);
  });

  it("rejects invalid portfolio project", () => {
    const result = portfolioProjectSchema.safeParse({ title: "", category: "Web" });
    expect(result.success).toBe(false);
  });
  ...
  it("validates client", () => {
    const result = clientSchema.safeParse({
      name: "Test Client",
      logo: "/clients/test.webp",
      url: "https://example.com",
    });
    expect(result.success).toBe(true);
  });

  it("validates process step", () => {
